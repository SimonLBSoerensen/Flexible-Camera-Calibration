...
calibrate_retval, cameraMatrix, distCoeffs, rvecs, tvecs, stdDeviationsIntrinsics, stdDeviationsExtrinsics, \
    ...
    armarkerIds_all, obj_points_all = np.load(os.path.join(train_folder,"out", "cali.npy"), allow_pickle=True)

image_shape = cv2.imread(glob.glob(os.path.join(train_folder, "*.jpg"))[0]).shape[:2]
