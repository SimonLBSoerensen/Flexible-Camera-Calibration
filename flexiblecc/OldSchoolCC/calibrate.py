...
def calibrate_camera_charuco(gray_imgs, squaresX, squaresY, squareLength, markerLength, dictionary, win_size=(10, 10),
    ...
    """
    Will do a charuco camera calibration. This will be done by finding the charuco boards in the provided grayscale
    images.

    Arguments:
        gray_imgs (iterable): Array there contains the images with chessboards. The images has to be in grayscale colorspace and in a "int" datatype
        squaresX (int): number of chessboard squares in X direction
        squaresY (int): number of chessboard squares in Y direction
        squareLength (float): chessboard square side length (normally in meters)
        markerLength (float): marker side length (same unit than squareLength)
        dictionary (opencv aruco_Dictionary): dictionary of markers indicating the type of markers. Can normally be found with cv2.aruco.getPredefinedDictionary
        win_size (tuple): Half of the side length of the search window
        zero_zone (tuple): Half of the size of the dead region in the middle of the search zone over which the summation
            in the formula below is not done. It is used sometimes to avoid possible singularities of
            the autocorrelation matrix. The value of (-1,-1) indicates that there is no such a size.
        ...
        verbose (int): 0, or 1. Verbosity mode.
                        0 = silent, 1 = progress bar and print out
        draw (str): The file path to save draw charuco images. If None it will not save any
    Returns:
        retval (float): The RMS re-projection error in pixels
        cameraMatrix (ndarray): Output 3x3 floating-point camera matrix
            [[fx, 0,  cx],
        ...
        stdDeviationsExtrinsics (): Standard deviations estimated for extrinsic parameters
        perViewErrors (): Vector of the RMS re-projection error estimated for each pattern view
        charucoCorners (ndarray): Vector of vectors of the charuco corners found in the images
        harucoIds (ndarray): Vector of vectors of the charuco corners id's found in the images
        markerCorners (ndarray): Vector of vectors of the charuco corners found in the images
        armarkerIds (ndarray): Vector of vectors of the charuco corners id's found in the images
        obj_points (ndarray): Vector of vectors of the charuco corners as object points found in the images
        board (aruco_CharucoBoard): The used charuco board
    """
    if criteria is None:
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
    else:
    ...
    return calibrate_retval, cameraMatrix, distCoeffs, rvecs, tvecs, stdDeviationsIntrinsics, stdDeviationsExtrinsics, perViewErrors, charucoCorners_all, charucoIds_all, markerCorners_all, armarkerIds_all, obj_points_all, board